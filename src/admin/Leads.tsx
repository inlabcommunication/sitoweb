...
export const Leads = () => {
  ...
  const updateLead = async (id: string, patch: Partial<Lead>) => {
    if (!db) return;
    await updateDoc(doc(db, 'leads', id), { ...patch, updated_at: new Date().toISOString() });
  ...
  const exportCsv = () => {
    ...
    const a = document.createElement('a'); a.href = url; a.download = `inlab-leads-${new Date().toISOString().split('T')[0]}.csv`; a.click();
  ...
  const counts = { all: leads.length, new: leads.filter((l) => l.status === 'new').length, contacted: leads.filter((l) => l.status === 'contacted').length, qualified: leads.filter((l) => l.status === 'qualified').length, closed: leads.filter((l) => l.status === 'closed').length };
    ...
    <div style={{ padding: '2rem', maxWidth: 1280, margin: '0 auto' }}>
      ...
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: '1rem' }}>
        {(['all', 'new', 'contacted', 'qualified', 'closed'] as const).map((s) => (
          <button key={s} onClick={() => setFilter(s)} style={{ padding: '6px 14px', borderRadius: 100, fontSize: 11, fontWeight: 500, letterSpacing: '.12em', textTransform: 'uppercase', border: '.5px solid var(--b)', background: filter === s ? 'var(--a)' : 'transparent', color: filter === s ? '#000' : 'var(--m)', cursor: 'pointer', transition: 'all .2s' }}>
      ...
      <div style={{ position: 'relative', marginBottom: '1.5rem' }}>
        ...
        <input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Cerca per email, nome o intento..." style={{ width: '100%', padding: '10px 14px 10px 38px', background: 'rgba(255,255,255,0.04)', border: '.5px solid var(--b)', borderRadius: 100, color: 'var(--t)', fontSize: 13, fontFamily: 'var(--fb)', outline: 'none' }} />
      </div>

      {loading ? (
        <div style={{ padding: '4rem', textAlign: 'center', color: 'var(--m)' }}>Caricamento...</div>
      ) : filtered.length === 0 ? (
        <div style={{ padding: '4rem 2rem', textAlign: 'center', color: 'var(--m)', background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 20 }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 8 }}>
          {filtered.map((l, i) => (
            <motion.div key={l.id} initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.02 }} onClick={() => setSelected(l)}
      ...
      <AnimatePresence>
        {selected && (
          <>
            <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} onClick={() => setSelected(null)} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.6)', zIndex: 100 }} />
            <motion.div initial={{ x: '100%' }} animate={{ x: 0 }} exit={{ x: '100%' }} transition={{ type: 'spring', damping: 28, stiffness: 280 }}
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                <Field icon={<Calendar size={14} />} label="Data">{new Date(selected.created_at).toLocaleString('it')}</Field>
                ...
                <div>
                  ...
                  <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap' }}>
                    {(['new', 'contacted', 'qualified', 'closed'] as const).map((s) => (
                      <button key={s} onClick={() => updateLead(selected.id, { status: s })} style={{ padding: '6px 12px', borderRadius: 100, fontSize: 11, fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', border: `.5px solid ${selected.status === s ? STATUS_COLOR[s] : 'var(--b)'}`, background: selected.status === s ? `${STATUS_COLOR[s]}22` : 'transparent', color: selected.status === s ? STATUS_COLOR[s] : 'var(--m)', cursor: 'pointer' }}>
                        {STATUS_LABEL[s]}
                      </button>
                    ))}
                  </div>
                ...
                <div>
                  ...
                  <textarea value={selected.notes ?? ''} onChange={(e) => setSelected({ ...selected, notes: e.target.value })} onBlur={() => updateLead(selected.id, { notes: selected.notes })} placeholder="Aggiungi note sulla trattativa..."
                    style={{ width: '100%', minHeight: 80, padding: 12, background: 'rgba(255,255,255,0.04)', border: '.5px solid var(--b)', borderRadius: 12, color: 'var(--t)', fontSize: 13, fontFamily: 'var(--fb)', outline: 'none', resize: 'vertical' }} />
                </div>
                {selected.conversation && selected.conversation.length > 0 && (
                  <div>
                    <div style={{ fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--m)', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}><MessageSquare size={12} />Conversazione chatbot</div>
                    <div style={{ background: 'var(--s)', border: '.5px solid var(--b)', borderRadius: 16, padding: '1rem', display: 'flex', flexDirection: 'column', gap: 8 }}>
                      {selected.conversation.map((m: any, i: number) => (
                        <div key={i} style={{ alignSelf: m.role === 'user' ? 'flex-end' : 'flex-start', maxWidth: '85%', padding: '8px 12px', borderRadius: m.role === 'user' ? '14px 14px 4px 14px' : '14px 14px 14px 4px', background: m.role === 'user' ? 'rgba(205,178,255,0.15)' : 'rgba(255,255,255,0.04)', fontSize: 12, lineHeight: 1.5, color: 'var(--t)' }}>
                          {m.content}
                        </div>
                      ))}
                    </div>
                ...
                <button onClick={() => deleteLead(selected.id)} style={{ marginTop: '1rem', padding: '10px 16px', background: 'rgba(255,100,100,0.08)', border: '.5px solid rgba(255,100,100,0.2)', borderRadius: 100, color: '#ff8888', fontSize: 11, fontWeight: 500, letterSpacing: '.1em', textTransform: 'uppercase', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 6, alignSelf: 'flex-start' }}>
...
const Field = ({ icon, label, children }: { icon: React.ReactNode; label: string; children: React.ReactNode }) => (
  <div>
    <div style={{ display: 'flex', alignItems: 'center', gap: 6, fontSize: 10, letterSpacing: '.15em', textTransform: 'uppercase', color: 'var(--m)', marginBottom: 4 }}>{icon}{label}</div>
