    ...
    <div>
      ...
      {(content.clients?.items || []).map((c: any, i: number) => (
        <CardBlock key={i} title={c.name}
          ...
          <Field label="Link" value={c.url} onChange={(v: string) => { const a = JSON.parse(JSON.stringify(content.clients.items)); a[i].url = v; set('clients.items', a); }}
            hint="URL sito oppure ID progetto interno (es: progetto/123)" />
      ...
      <AddBtn onClick={() => setContent((p: any) => {
        ...
        n.clients.items.push({ name: 'Nuovo cliente', url: '', logo: '' }); return n;
